...
for i in range(0, len(ROpt)):
    ...
    for deltaValue in deltaOptions:
                ...
                for p in prof_keys:
                    try:
                        ...
                        if ((Conj_Lin[lin]['Supervision'] in Conj_E[p]) and
                            ...
                            m.addConstr(x_var_des[p][Conj_Lin[lin]['Supervision']], gp.GRB.LESS_EQUAL,
                                        x_var_des[p][Conj_Lin[lin]['Examen']], name='')
                        else:
                            m.addConstr(x_var_des[p][Conj_Lin[lin]['Supervision']], gp.GRB.LESS_EQUAL,
                                        0, name='')
                            m.addConstr(x_var_des[p][Conj_Lin[lin]['Examen']], gp.GRB.LESS_EQUAL,
                                        0, name='')
                ...
                for k in Conj_Corr:
                    if ((Conj_B[k]['Practica'] == pr) and
                        ...
                        for p in prof_keys:
                            try:
                                #print(x_var_des[p][k])
                                #cond_4_1[j][pr].addTerms(1, x_var_des[p][k])
                                if (('Practica - I' in pr) or
                                ('Practica Profesional' in pr)):
        ...
        try:
            mexp.F_export_model_results(G, Conj_B, Conj_U, Conj_S, Conj_Lin, X, Y,
