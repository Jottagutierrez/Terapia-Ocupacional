...
def F_translate_into_week(sheet_list):
    Result = collections.namedtuple('Result',['week_L','pract_L', 'act_L', 'cent_L', 'est_L'])
    ...
    for i in range(1, sheet_cent.nrows):
        ...
        for j in range(1, sheet_cent.ncols):
            cent_list[sheet_cent.cell(i,0).value][sheet_cent.cell(0,j).value] = sheet_cent.cell(i,j).value
        ...
        while i in range(pract_list[p]['Semana Inicio'], pract_list[p]['Semana Termino']+1):                
            ...
            for k in num_est[p].keys():
                for j in range(0, num_est[p][k][current_rot]):
                    act_list.append({'Practica': p,'Tipo': 'Supervision', 'Centro': k, 'Semana': sem_sup[p], 'Tiempo': act_info['Supervision']['Tiempo'], 'Especialidad': cent_list[k]['Especialidad'], 'Rotativa': current_rot})
                    try:
                        for c in sem_corr[p]:
                            act_list.append({'Practica': p,'Tipo': 'Correccion', 'Centro': k, 'Semana': c, 'Tiempo': act_info['Correccion']['Tiempo'], 'Especialidad': cent_list[k]['Especialidad'], 'Rotativa': current_rot})
                    except TypeError:
                        act_list.append({'Practica': p,'Tipo': 'Correccion', 'Centro': k, 'Semana': sem_corr[p], 'Tiempo': act_info['Correccion']['Tiempo'], 'Especialidad': cent_list[k]['Especialidad'], 'Rotativa': current_rot})
                    try:
                        act_list.append({'Practica': p,'Tipo': 'Examen', 'Centro': k, 'Semana': sem_exam[p], 'Tiempo': act_info['Examen']['Tiempo'], 'Especialidad': cent_list[k]['Especialidad'], 'Rotativa': current_rot})
                    except KeyError:
                        pass
            current_rot+=1 
            i+=rot_info[p]
    
    R = Result(week_list, pract_list, act_list, cent_list, num_est)                
    return R;
######################################################
...
def F_create_param_file(act_list, prof_list, cent_list,
    ...
    for j in cent_keys:
        Conj_A[j] = []
        for k in act_keys:
            if (act_list[k]['Centro'] == j and
                act_list[k]['Tipo'] == 'Supervision'):
                Conj_A[j].append(k)    
    ...
    for p in prof_list.keys():        
        Conj_E[p] = []
        for k in act_keys:
            if (act_list[k]['Tipo'] == 'Correccion' or
