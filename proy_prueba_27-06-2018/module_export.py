...
######################################################
def F_student_rotative(X, Act_List, tb_pract, tb_act, prof, cent, rot):
    pract_info = {
    ...
    est_count = []
    for k in X[prof]:
        if ((Act_List[k]['Centro'] == cent) and (Act_List[k]['Rotativa'] == rot)):
            for pr in pract_info.keys():
                if (pr == tb_pract):
                    for act_type in pract_info[pr]:
                        if (act_type == tb_act):
                            if ((Act_List[k]['Practica'] == pr) and
                                (Act_List[k]['Tipo'] == act_type)):
                                est_count.append((Act_List[k]['Linea']))
    est_count = len(list(set(est_count)))
    return est_count

def F_cent_student_row(X, Act_List, tb_pract, tb_act, prof, cent_asigned):
    cent_row = {}
    for cent in cent_asigned:
        cent_row[cent] = {}
        est_sum = 0
        for rot in range(1, 7):
            cent_row[cent][rot] = F_student_rotative(X, Act_List, tb_pract, tb_act,
                    prof, cent, rot)
            est_sum = est_sum + cent_row[cent][rot]
        cent_row[cent]['Total'] = est_sum
    return cent_row

def F_export_model_results(G, Act_List, Week_List, Conj_U, Conj_Lin, X, Y, W,
    ...
    cell_format = {'Normal': workbook.add_format(),
                   ...
                   'Title_BG_Centered_Lime': workbook.add_format(),
    ...
    cell_format['Title_BG_Centered_Lime'].set_bg_color('#ADE570')
    ...
    worksheet.merge_range(0, 0, 1, 0, 'Nombre CDP', cell_format['Title_BG_Centered_Lime'])
    worksheet.merge_range(0, 1, 1, 1, 'Especialidad', cell_format['Title_BG_Centered_Lime'])
    worksheet.merge_range(0, 2, 1, 2, 'Docente Supervisor', cell_format['Title_BG_Centered_Lime'])
    worksheet.merge_range(0, 3, 0, 5, 'Docente Corrector', cell_format['Title_BG_Centered_Lime'])
    worksheet.write(1, 3, '4to', cell_format['Title_BG_Centered_Lime'])
    worksheet.write(1, 4, '5to', cell_format['Title_BG_Centered_Lime'])
    worksheet.write(1, 5, 'Examen', cell_format['Title_BG_Centered_Lime'])
    ...
    for c in cent_keys:
        ...
        for p in X.keys():
            for act in X[p]:
                if Act_List[act]['Centro'] == c:
                    if Act_List[act]['Tipo'] == 'Correccion':
                        if (Act_List[act]['Practica'] == 'Practica - I' or
                            Act_List[act]['Practica'] == 'Practica Profesional'):
                            p_corr['4to'].append(p)
                        elif (Act_List[act]['Practica'] == 'Internado' or
                            Act_List[act]['Practica'] == 'Mencion'):
                            p_corr['5to'].append(p)
                    elif Act_List[act]['Tipo'] == 'Examen':
                        p_ex.append(p)
    ...
    for p in X.keys():
            ...
            for k in Conj_U[s]: #en todas las actividades de esa semana
                for act in X[p]:#todas las atividades realizados por el profesor
                    if k == int(act): #si la actividad realizada corresponde a las actividades realizadas esa semana
                        ...
                        worksheet.write(row, 2,Act_List[k]['Tipo'])
                        worksheet.write(row, 3,', '.join(Act_List[k]['Especialidad']))
                        worksheet.write(row, 4,Act_List[k]['Centro'])
                        worksheet.write(row, 5,Act_List[k]['Practica'])
                        worksheet.write(row, 6,Act_List[k]['Rotativa'])
                        row += 1
        ...
        cent_asigned = []
        for act in X[p]:
            cent_asigned.append(Act_List[act]['Centro'])
        ...
        for tb in table_list.keys():
            ...
            cent_row = F_cent_student_row(X, Act_List, table_list[tb]['Practica'],
            ...
            for pr in rot_info.keys():
                if ((total_tb > 0) and (pr in tb)):
                    ...
                    if (rot_info[pr] < 4):
                        worksheet.merge_range(row, col+1, row+1, col+4, 'Alumnos', cell_format['Title_BG_Centered_LightBlue'])
                        worksheet.merge_range(row, col+5, row+1, col+5, 'Total por Centro', cell_format['Title_BG_Centered_LightBlue'])
                        worksheet.merge_range(row, col+6, row+1, col+6, 'Total', cell_format['Title_BG_Centered_LightBlue'])
                    else:
                    ...
                    for cent in cent_row.keys():
                        if (cent_row[cent]['Total'] > 0):
                            ...
                            if (rot_info[pr] < 4):
                                worksheet.merge_range(row, col+1, row, col+4, cent_row[cent][1], cell_format['Normal_Centered'])
                                worksheet.write(row, col+5, cent_row[cent]['Total'], cell_format['Normal_Centered'])
                            else:
                    ...
                    if (cent_count > 1):
                        if (rot_info[pr] < 4):
                            worksheet.merge_range(row_i, col+6, row-1, col+6, total_tb, cell_format['Normal_Centered'])
                        else:
                            worksheet.merge_range(row_i, col+rot_info[pr]+2, row-1, col+rot_info[pr]+2, total_tb, cell_format['Normal_Centered'])
                    else:
                        if (rot_info[pr] < 4):
                            worksheet.write(row_i, col+6, total_tb, cell_format['Normal_Centered'])
                        else:
