...
def correr(deltaOptions):
    ...
    for deltaValue in deltaOptions:
        ...
        list_horasna[deltaValue] = Obj['Arg_1'].getValue()
        ######################################
    
    pl.plot(listifyDict(list_sobrecarga), listifyDict(list_horasna), ROpt['Plot'])
    pl.xlabel('Excedente profesores internos [hrs]')
    pl.ylabel('Costo externalización [CLP]')
        
    
    SobrecargaT = listifyDict(list_sobrecarga)
    HorasnaT = listifyDict(list_horasna)
    print(SobrecargaT)
    print(HorasnaT)
    
    return SobrecargaT, HorasnaT, deltaOptions, G, Act_List, Conj_U, Week_List, Conj_Lin, X, Y, W, week_keys, cent_keys, cent_info
    #SobrecargaT, CostoT, deltaOptions, Act_List, Conj_U, Conj_S, X, Y, week_keys
